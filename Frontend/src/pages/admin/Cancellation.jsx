...
const Cancellation = () => {
  ...
  useEffect(() => {
    const fetchDoc = async () => {
      const res = await getLegalDocBySlug("cancellation")
      if (res.success && res.data) {
        setContent(res.data.content)
      } else {
        setContent(`Cancellation policy content goes here...`)
      }
      setLoading(false)
    }
    fetchDoc()
  ...
  if (loading) return (
    <div className="flex flex-col justify-center items-center h-64 gap-4">
        ...
        <p className="text-[#a6ba9c] text-xs font-black uppercase tracking-widest animate-pulse">Loading Policy...</p>
    </div>
  )
  ...
  return (
    <div className="space-y-10 pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
            <h2 className="text-white text-3xl font-black uppercase tracking-tighter leading-none mb-2">Cancellation <br/>Policy</h2>
            <p className="text-[#a6ba9c] text-xs font-semibold opacity-60">Manage your ticket cancellation and refund protocols.</p>
        </div>
        <button
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-[#1c2619] border border-[#2e3928] rounded-[32px] overflow-hidden shadow-2xl p-8 space-y-8">
        ...
        <div className="relative group">
            <textarea 
                ...
                placeholder="Type your cancellation policy here..."
            />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
                { label: "Recovery", val: "90%", color: "text-[#59f20d]" },
                { label: "Window", val: "24 HOURS", color: "text-white" },
